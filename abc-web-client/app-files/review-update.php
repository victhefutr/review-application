...
$sqla = "SELECT review FROM reviews WHERE review_id = ? AND username = ?";

if($stmta = mysqli_prepare($link_to_db, $sqla))
{
mysqli_stmt_bind_param($stmta, "ss", $value_00_review_id, $value_00_username);
...
$form_succ = "Thank you for updating your review, you will be redirected to the home page in 3 seconds";
...
{
//ERROR ----
$form_err = "The review you are trying to update doesn't exist anymore";
//ERROR ----
}
} 
...
<title>Update your review - ABC Travels</title>
...
<?php
$header_caption = "Use the form below to update your review";
...
$review_data_sql= "SELECT review, rating FROM reviews WHERE review_id='$review_id' AND username = '$get_username'";
...
<!--CONTENT-->
<div class="form-header">Update Review</div>

<form method="post" action="review-update?id=<?php echo $review_id; ?>">
<!--FORM INPUTS-->
...
<textarea class="form-control" type="text" name="xx_review" placeholder="What do you think about ABC Travels <?php echo $disp_full_name; ?>" maxlength="128" rows="4"><?php echo $stored_review; ?></textarea>
...
<option value="5" <?php if($stored_rating == "5") { echo "selected"; } ?>>5 Stars</option>
<option value="4" <?php if($stored_rating == "4") { echo "selected"; } ?>>4 Stars</option>
<option value="3" <?php if($stored_rating == "3") { echo "selected"; } ?>>3 Stars</option>
<option value="2" <?php if($stored_rating == "2") { echo "selected"; } ?>>2 Stars</option>
<option value="1" <?php if($stored_rating == "1") { echo "selected"; } ?>>1 Star</option>
</select>
...
<div class="form-group">
<button class="button-true" name="submit" type="submit" value="update">Update</button>
</div>
